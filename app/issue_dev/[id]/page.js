...
const IssueDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchIssueById = async (issueId) => {
      try {
        // Call the RPC function with the provided issue ID
        ...
        if (error) {
          ...
          setIssue(data[0]); // Data will be an array with a single object
          console.log("Fetched issue:", data[0]);
        }
      } catch (err) {
    ...
    fetchIssueById(id); // Pass `id` here to fetch the specific issue
  }, [id]);

  const handleStatusChange = async (newStatus) => {
    try {
      const { error } = await supabase.rpc("update_issue", {
        issue_id_input: id,       // Pass the issue ID
        new_status: newStatus     // Pass the new status
      });
  ...
  return (
    <div>
      <div className="p-8 max-w-2xl mx-auto bg-gray-50 rounded-lg shadow-md mt-10">
        <Link href="/home_dev" className="text-gray-500 hover:text-gray-700 mb-6 inline-block">
